...
void setup() {
  ...
  /*for (int i = 0; i < arrows.length; i++) {
    ...
    translate(340, arrows[i].y);
      ...
      TColor console = TColor.newHSV(arrows[i].h, nesSat[i], nesBright[i]);
      fill(console.hue(), console.saturation(), console.brightness());
      arrows[i].drawArrows();
    popMatrix();
  }
  ...
  for (int i = 0; i < arrows.length; i++) {
    ...
    translate(0, 340+ arrows[i].y);
      noStroke();
      arrows[i].h = psHues[i];
      TColor console = TColor.newHSV(arrows[i].h, psSat[i], psBright[i]);
      fill(console.hue(), console.saturation(), console.brightness());
      arrows[i].drawArrows();
    popMatrix();
  }
  ...
  for (int i = 0; i < arrows.length; i++) {
    pushMatrix();
      ...
      TColor console = TColor.newHSV(arrows[i].h, xboxSat[i], xboxBright[i]);
      fill(console.hue(), console.saturation(), console.brightness());
      arrows[i].drawArrows();
    popMatrix();
  }*/
